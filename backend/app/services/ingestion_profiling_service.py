    ...
    def profile_dataset(
        ...
        *,
        ...
        try:
            ...
            raise IngestionProfilingError(f"File parsing failed: {exc}") from exc
        ...
        try:
            ...
            raise IngestionProfilingError(f"Column name normalization failed: {exc}") from exc
        ...
        for row_number, raw_row in enumerate(raw_rows):
            ...
            for ci, cell in enumerate(raw_row):
                ...
                try:
                    ...
                    raise IngestionProfilingError(
                        f"Row {row_number}, column '{orig_name}': value conversion failed: {exc}"
                    ) from exc
        ...
        try:
            ...
            raise IngestionProfilingError(f"Consistency validation failed: {exc}") from exc
...
def _validate_consistency(
    ...
    if len(set(norm_names)) != len(norm_names):
        raise IngestionProfilingError(f"Duplicate normalized column names detected: {norm_names}")
    ...
    if row_numbers != list(range(n_rows)):
        raise IngestionProfilingError(f"Row numbers are not contiguous from 0: {row_numbers[:10]}…")
    ...
    for row in profiled_rows:
        if set(row.values.keys()) != expected_keys:
            ...
            raise IngestionProfilingError(
                f"Row {row.row_number} has incorrect key set. Missing: {missing}. Extra: {extra}."
            )
