...
class RowErrorSchema(BaseModel):
    ...
    row_number: Annotated[
        ...
        Field(
            ge=1,
            description=(
                "1-based row number in the CSV file (header row = 0, first data row = 1)."
            ),
        ),
    ]
    column: Annotated[
...
class CsvPreviewResponse(BaseModel):
    ...
    sample_records: Annotated[
        list[SampleRecordSchema],
        Field(
            description="Up to 10 valid rows for display in the preview table.",
        ),
    ]
    conflicts: Annotated[
        list[CsvConflictSchema],
        Field(
            description=(
