...
async def main():
    ...
    async with engine.connect() as conn:
        ...
        res = await conn.execute(
            text(
                "SELECT to_regclass('public.dashboards') as dashboards, to_regclass('public.dashboard_cards') as dashboard_cards"
            )
        )
        row = res.first()
        ...
        fk_q = text(
            "SELECT conname, confrelid::regclass::text AS referenced_table, conrelid::regclass::text AS table_name, pg_get_constraintdef(oid) AS def FROM pg_constraint WHERE contype='f' AND conrelid IN (SELECT oid FROM pg_class WHERE relname IN ('dashboard_cards'))"
        )
        ...
        # check indexes
        idx_q = text(
            "SELECT indexname, indexdef FROM pg_indexes WHERE tablename IN ('dashboards','dashboard_cards')"
        )
