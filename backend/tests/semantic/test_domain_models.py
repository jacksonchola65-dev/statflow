import pytest
from app.semantic.domain_models import DomainEvidence, DomainPrediction, DomainScore
from app.semantic.semantic_types import DatasetDomain, SemanticType


def test_valid_domain_evidence():
    ev = DomainEvidence(
        domain=DatasetDomain.GENERAL, semantic_type=SemanticType.EMAIL, weight=1.5, description="d"
    )
...
def test_nan_and_infinite_weight_rejected():
    with pytest.raises(ValueError):
        DomainEvidence(
            domain=DatasetDomain.GENERAL, semantic_type=SemanticType.EMAIL, weight=float("nan")
        )
    with pytest.raises(ValueError):
        DomainEvidence(
            domain=DatasetDomain.GENERAL, semantic_type=SemanticType.EMAIL, weight=float("inf")
        )


def test_valid_domain_score_and_evidence_ordering():
    ev1 = DomainEvidence(
        domain=DatasetDomain.FINANCE, semantic_type=SemanticType.CURRENCY, weight=1.0
    )
    ev2 = DomainEvidence(
        domain=DatasetDomain.FINANCE, semantic_type=SemanticType.CURRENCY, weight=2.0
    )
...
def test_domain_prediction_alternatives_sorting_and_primary_excluded():
    ...
    pred = DomainPrediction(
        primary_domain=DatasetDomain.FINANCE,
        confidence=0.85,
        alternatives=(ds1, ds2, ds3),
        evidence=(
            DomainEvidence(
                domain=DatasetDomain.FINANCE, semantic_type=SemanticType.CURRENCY, weight=1.0
            ),
    ...
    assert (
        pred.alternatives[0].domain == DatasetDomain.HEALTHCARE
        or pred.alternatives[0].domain == DatasetDomain.GENERAL
        or pred.alternatives[0].domain == DatasetDomain.HEALTHCARE
    )
