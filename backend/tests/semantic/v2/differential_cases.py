...
def generate_random_cases(
    seed: int, n: int, min_vals: int = 0, max_vals: int = 50
) -> List[ColumnCase]:
    ...
    def rnd_value(kind: int) -> Optional[str]:
        ...
        if kind == 4:
            ...
            return " ".join(
                rnd.choice(["foo", "bar", "baz", "qux", "テスト", "ñ"]) for _ in range(length)
            )
        if kind == 5:
    ...
    for i in range(n):
        ...
        for _ in range(count):
            kind = rnd.choices(
                range(9), weights=[0.05, 0.2, 0.15, 0.05, 0.25, 0.05, 0.05, 0.15, 0.05]
            )[0]
