...
def sc(t, c):
    return SemanticClassification(semantic_type=t, confidence=c, evidence=(SemanticEvidence(source="d", score=float(c)),), detector="det")
...
def test_semantic_profile_integration_scenarios_and_serialization():
    ...
    for entity_cols, key_cols, rel_cols, expected_domain in scenarios:
        ...
        keys = EntityKeyDetector.discover(EntityKeyDetectionInput(entities=entities, columns=tuple(key_cols)))
        rels = RelationshipDetector.discover(RelationshipDetectionInput(entities=entities, keys=keys, columns=tuple(rel_cols)))
        ...
        columns = tuple(ColumnClassification(column_name=f"c{i}", classifications=(sc(SemanticType.UNKNOWN, 0.5),)) for i in range(max(1, len(key_cols) or 1)))

        profile = SemanticProfileBuilder.compose(domain, entities, rels, keys, None or __import__('app.semantic.analytics_role_models', fromlist=['AnalyticsRoleProfile']).AnalyticsRoleProfile(), columns)
        ...
        sm = __import__('app.semantic.semantic_models', fromlist=['SemanticEntity', 'SemanticRelationship'])
        name_to_id = {}
        sem_entities = []
        for e in entities:
            eid = uuid4()
            name = e.name if hasattr(e, 'name') else getattr(e, 'id', str(e))
            name_to_id[getattr(e, 'name', str(e))] = eid
            sem_entities.append(sm.SemanticEntity(id=eid, name=getattr(e, 'name', 'entity'), semantic_type=SemanticType.UNKNOWN))
        ...
        for r in rels:
            ...
            if hasattr(r, 'source_entity'):
                ...
                sem_rels.append(sm.SemanticRelationship(source_entity_id=sid, target_entity_id=tid, relationship_type=getattr(r, 'relationship_type', 'rel')))
            else:
                # already semantic relationship
                sem_rels.append(r)

        model = SemanticProfileModel(domain=profile.domain, entities=tuple(sem_entities), relationships=tuple(sem_rels), columns=profile.columns, analytics_roles=profile.analytics_roles)
        d = to_dict(model)
...
def test_full_pipeline_performance_and_behavior_large():
    ...
    for _ in range(5):
        ...
        keys = EntityKeyDetector.discover(EntityKeyDetectionInput(entities=entities, columns=tuple(key_cols)))
        rels = RelationshipDetector.discover(RelationshipDetectionInput(entities=entities, keys=keys, columns=tuple(rel_cols)))
        _ = SemanticProfileBuilder.compose(DomainDetectionResult(domain=DatasetDomain.GENERAL), entities, rels, keys, __import__('app.semantic.analytics_role_models', fromlist=['AnalyticsRoleProfile']).AnalyticsRoleProfile(), tuple(ColumnClassification(column_name=f"c{i}", classifications=(sc(SemanticType.UNKNOWN, 0.5),)) for i in range(n)))
