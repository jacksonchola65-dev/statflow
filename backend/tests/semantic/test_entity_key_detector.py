...
def test_identifier_detection():
    ...
    col = EntityKeyColumnInput(column_name="customer_id", classifications=(sc(SemanticType.IDENTIFIER, 0.9),), uniqueness_ratio=0.85, null_ratio=0.01)
...
def test_integer_fallback_detection():
    ...
    col = EntityKeyColumnInput(column_name="order", classifications=(sc(SemanticType.INTEGER, 0.95),), uniqueness_ratio=0.96, null_ratio=0.0)
    inp = EntityKeyDetectionInput(entities=(ent,), columns=(col,))
    res = EntityKeyDetector.discover(inp)
    assert len(res) == 1
    r = res[0]
...
def test_text_fallback_detection_and_thresholds():
    ...
    col_bad = EntityKeyColumnInput(column_name="doc_key", classifications=(sc(SemanticType.TEXT, 0.9),), uniqueness_ratio=0.9, null_ratio=0.0)
    col_good = EntityKeyColumnInput(column_name="doc_key", classifications=(sc(SemanticType.TEXT, 0.9),), uniqueness_ratio=0.96, null_ratio=0.0)
...
def test_confidence_threshold_and_null_threshold():
    ...
    col = EntityKeyColumnInput(column_name="u_id", classifications=(sc(SemanticType.IDENTIFIER, 0.59),), uniqueness_ratio=0.9, null_ratio=0.0)
    inp = EntityKeyDetectionInput(entities=(ent,), columns=(col,))
    assert EntityKeyDetector.discover(inp) == ()
...
def test_name_normalization_and_suffix_removal_and_unmatched_ignored():
    ...
    col_match = EntityKeyColumnInput(column_name=" employee-code ", classifications=(sc(SemanticType.IDENTIFIER, 0.9),), uniqueness_ratio=0.9, null_ratio=0.0)
    col_unmatched = EntityKeyColumnInput(column_name="dept_id", classifications=(sc(SemanticType.IDENTIFIER, 0.9),), uniqueness_ratio=0.9, null_ratio=0.0)
...
def test_duplicate_entity_name_first_match():
    ...
    col = EntityKeyColumnInput(column_name="client_id", classifications=(sc(SemanticType.IDENTIFIER, 0.9),), uniqueness_ratio=0.9, null_ratio=0.0)
...
def test_highest_classification_selected_and_tie_break():
    ...
    c1 = SemanticClassification(semantic_type=SemanticType.INTEGER, confidence=0.9, evidence=(SemanticEvidence(source="d", score=0.9),))
    c2 = SemanticClassification(semantic_type=SemanticType.TEXT, confidence=0.9, evidence=(SemanticEvidence(source="d", score=0.9),))
    col = EntityKeyColumnInput(column_name="x", classifications=(c1, c2), uniqueness_ratio=0.96, null_ratio=0.0)
    inp = EntityKeyDetectionInput(entities=(ent,), columns=(col,))
    res = EntityKeyDetector.discover(inp)
    assert len(res) == 1
...
def test_confidence_clamping_and_evidence_preserved_and_input_immutability():
    ...
    col = EntityKeyColumnInput(column_name="big_id", classifications=(sc(SemanticType.IDENTIFIER, 1.0),), uniqueness_ratio=1.0, null_ratio=0.0)
    inp_cols = (col,)
...
def test_deterministic_sorting():
    ...
    c1 = EntityKeyColumnInput(column_name="A_id", classifications=(sc(SemanticType.IDENTIFIER, 0.9),), uniqueness_ratio=0.9, null_ratio=0.0)
    c2 = EntityKeyColumnInput(column_name="B_id", classifications=(sc(SemanticType.IDENTIFIER, 0.9),), uniqueness_ratio=0.9, null_ratio=0.0)
...
def test_performance_100x100():
    ...
    for i in range(100):
        ...
        cols.append(EntityKeyColumnInput(column_name=f"ent_{i}_id", classifications=tuple(cset), uniqueness_ratio=0.98, null_ratio=0.01))
