...
def test_person_entity_detection_and_suffix_normalization():
    cols = [
        EntityColumnInput(column_name=" customer_name ", classifications=(sc(SemanticType.PERSON, 0.9),)),
        EntityColumnInput(column_name="customer-id", classifications=(sc(SemanticType.PERSON, 0.85),)),
    ]
...
def test_organization_and_geographic_detection_and_grouping():
    cols = [
        EntityColumnInput(column_name="org_name", classifications=(sc(SemanticType.ORGANIZATION, 0.9),)),
        EntityColumnInput(column_name="company", classifications=(sc(SemanticType.ORGANIZATION, 0.88),)),
        EntityColumnInput(column_name="city", classifications=(sc(SemanticType.CITY, 0.95),)),
        EntityColumnInput(column_name="province ", classifications=(sc(SemanticType.PROVINCE, 0.8),)),
    ]
    res = EntityCandidateDetector.discover(cols)
...
def test_underscore_hyphen_and_whitespace_normalization_and_empty_name_fallback():
    cols = [
        ...
        EntityColumnInput(column_name="employee__id", classifications=(sc(SemanticType.PERSON, 0.85),)),
    ]
    res = EntityCandidateDetector.discover(cols)
    assert len(res) >= 1
...
def test_multiple_semantic_types_and_duplicate_removal_and_evidence_ordering():
    cols = [
        EntityColumnInput(column_name="entity_name", classifications=(sc(SemanticType.PERSON, 0.9), sc(SemanticType.ORGANIZATION, 0.7))),
        EntityColumnInput(column_name="entity_id", classifications=(sc(SemanticType.PERSON, 0.85), sc(SemanticType.COUNTRY, 0.65))),
    ]
    res = EntityCandidateDetector.discover(cols)
    assert len(res) == 1
    rc = res[0]
...
def test_deterministic_sorting_and_input_immutability():
    cols = []
    for i in range(10):
        cols.append(EntityColumnInput(column_name=f"name_{i}", classifications=(sc(SemanticType.PERSON, 0.9 - i * 0.01),)))
...
def test_performance_100_columns():
    ...
    for i in range(100):
        ...
        for j in range((i % 3) + 1):
            cset.append(SemanticClassification(semantic_type=types[(i + j) % len(types)], confidence=0.9, evidence=(SemanticEvidence(source="d", score=0.9, description="e"),), detector="d"))
