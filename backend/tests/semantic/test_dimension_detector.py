...
def test_dimension_mapping_all_types():
    cols = [
        ...
        DimensionColumnInput(column_name="country", classifications=(sc(SemanticType.COUNTRY, 0.7),)),
        DimensionColumnInput(column_name="city", classifications=(sc(SemanticType.CITY, 0.7),)),
        DimensionColumnInput(column_name="province", classifications=(sc(SemanticType.PROVINCE, 0.7),)),
        DimensionColumnInput(column_name="district", classifications=(sc(SemanticType.DISTRICT, 0.7),)),
        DimensionColumnInput(column_name="person", classifications=(sc(SemanticType.PERSON, 0.7),)),
        DimensionColumnInput(column_name="org", classifications=(sc(SemanticType.ORGANIZATION, 0.7),)),
        ...
        DimensionColumnInput(column_name="category", classifications=(sc(SemanticType.CATEGORY, 0.7),)),
        DimensionColumnInput(column_name="text", classifications=(sc(SemanticType.TEXT, 0.7),), cardinality_ratio=0.2),
    ]
...
def test_thresholds_and_null_ratio():
    c1 = DimensionColumnInput(column_name="low", classifications=(sc(SemanticType.DATE, 0.7),), null_ratio=0.5)
    c2 = DimensionColumnInput(column_name="high", classifications=(sc(SemanticType.DATE, 0.7),), null_ratio=0.51)
    c3 = DimensionColumnInput(column_name="lowconf", classifications=(sc(SemanticType.DATE, 0.59),),)
...
def test_text_cardinality_rule():
    c1 = DimensionColumnInput(column_name="text_ok", classifications=(sc(SemanticType.TEXT, 0.7),), cardinality_ratio=0.5)
    c2 = DimensionColumnInput(column_name="text_bad", classifications=(sc(SemanticType.TEXT, 0.7),), cardinality_ratio=0.51)
...
def test_unsupported_types_ignored():
    c1 = DimensionColumnInput(column_name="curr", classifications=(sc(SemanticType.CURRENCY, 0.9),),)
    c2 = DimensionColumnInput(column_name="qty", classifications=(sc(SemanticType.QUANTITY, 0.9),),)
    c3 = DimensionColumnInput(column_name="pct", classifications=(sc(SemanticType.PERCENTAGE, 0.9),),)
...
def test_duplicate_suppression_first_occurrence_wins():
    c1 = DimensionColumnInput(column_name="dup", classifications=(sc(SemanticType.COUNTRY, 0.7),),)
    c2 = DimensionColumnInput(column_name="dup", classifications=(sc(SemanticType.CITY, 0.9),),)
    result = DimensionDetector.discover((c1, c2))
    assert len(result) == 1
...
def test_evidence_preservation():
    ...
    c = DimensionColumnInput(column_name="x", classifications=(SemanticClassification(semantic_type=SemanticType.DATE, confidence=0.8, evidence=(evidence,),),),)
    result = DimensionDetector.discover((c,))
    assert result[0].evidence == (evidence,)


def test_sorting_and_immutability():
    c1 = DimensionColumnInput(column_name="b", classifications=(sc(SemanticType.COUNTRY, 0.8),),)
    c2 = DimensionColumnInput(column_name="a", classifications=(sc(SemanticType.CITY, 0.8),),)
    c3 = DimensionColumnInput(column_name="c", classifications=(sc(SemanticType.CATEGORY, 0.7),),)
    cols = [c1, c2, c3]
