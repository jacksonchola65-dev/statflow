...
async def test_data_points_ordered_by_year_then_created_at(
    ...
    _ = await _make_data_point(db_session, ds.id, ind.id, 2023)
    _ = await _make_data_point(db_session, ds.id, ind.id, 2021)
    _ = await _make_data_point(db_session, ds.id, ind.id, 2022)
...
@pytest.mark.asyncio
async def test_filter_by_dataset_id(authed_client: AsyncClient, db_session: AsyncSession) -> None:
    cat = await _make_category(db_session)
    ind = await _make_indicator(db_session, cat.id)
...
@pytest.mark.asyncio
async def test_filter_by_indicator_id(authed_client: AsyncClient, db_session: AsyncSession) -> None:
    cat = await _make_category(db_session)
...
@pytest.mark.asyncio
async def test_filter_by_province_id(authed_client: AsyncClient, db_session: AsyncSession) -> None:
    cat = await _make_category(db_session)
    ind = await _make_indicator(db_session, cat.id)
    ds = await _make_dataset(db_session)
...
@pytest.mark.asyncio
async def test_filter_by_district_id(authed_client: AsyncClient, db_session: AsyncSession) -> None:
    cat = await _make_category(db_session)
    ind = await _make_indicator(db_session, cat.id)
    ds = await _make_dataset(db_session)
...
@pytest.mark.asyncio
async def test_province_and_district_together_returns_422(authed_client: AsyncClient) -> None:
    prov_id = uuid.uuid4()
    dist_id = uuid.uuid4()
    response = await authed_client.get(
