...
async def test_successful_confirmation_persists_dataset(authed_client, db_session):
    ...
    ds = (
        (
            await db_session.execute(
                select(UniversalDataset).where(
                    UniversalDataset.id == uuid.UUID(data2["dataset_id"])
                )
            )
        )
        .scalars()
    ...
    ver = (
        (
            await db_session.execute(
                select(UniversalDatasetVersion).where(
                    UniversalDatasetVersion.id == uuid.UUID(data2["version_id"])
                )
            )
        )
        .scalars()
    ...
    rows = (
        (
            await db_session.execute(
                select(UniversalDatasetRow).where(UniversalDatasetRow.dataset_version_id == ver.id)
            )
        )
        .scalars()
        .all()
...
async def test_consumed_token_cannot_be_reused(authed_client):
    ...
    resp2 = await authed_client.post(
        URL_CONFIRM, json={"mapped_preview_token": mp, "name": "X", "description": None}
    )
    assert resp2.status_code == 201

    # second attempt should return 404 (token not found)
    resp3 = await authed_client.post(
        URL_CONFIRM, json={"mapped_preview_token": mp, "name": "X2", "description": None}
    )
...
async def test_missing_token_returns_404(authed_client):
    resp = await authed_client.post(
        URL_CONFIRM,
        json={"mapped_preview_token": str(uuid.uuid4()), "name": "A", "description": None},
    )
...
async def test_expired_token_returns_410(authed_client):
    ...
    resp2 = await authed_client.post(
        URL_CONFIRM, json={"mapped_preview_token": mp, "name": "A", "description": None}
    )
...
async def test_wrong_owner_returns_403(authed_client):
    ...
    resp = await authed_client.post(
        URL_CONFIRM, json={"mapped_preview_token": tok, "name": "A", "description": None}
    )
...
async def test_blank_name_rejected(authed_client):
    ...
    resp = await authed_client.post(
        URL_CONFIRM,
        json={"mapped_preview_token": str(uuid.uuid4()), "name": "", "description": None},
    )
...
async def test_persistence_failure_rolls_back_and_token_remains(authed_client, monkeypatch):
    ...
    monkeypatch.setattr(
        imports_mod.UniversalDatasetPersistenceService, "create_dataset_from_rows", _fail_create
    )

    resp2 = await authed_client.post(
        URL_CONFIRM, json={"mapped_preview_token": mp, "name": "FailTest", "description": None}
    )
